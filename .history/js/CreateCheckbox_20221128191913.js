class GetProducts{
    ...
    updateHtmlPage(){
        ...
        var startDiv = `<div><<label for="product">Product<label><input type="checkbox" name="${product[i].name}"`
        var endDiv = `</input></div>`
        
        for(var i = 0; i < product.length; i++){
            var brandsOptionsHTML = brandsOptionsHTML+`<option value="${brands[i].id}">${brands[i].name}</option>`
            var productCheckhtml= productCheckhtml+`<input type="checkbox" name="${product[i].name}" value="${product[i].id}" id="${product[i].id}"></input>`
